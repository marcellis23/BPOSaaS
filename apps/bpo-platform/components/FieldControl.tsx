...
function RepeaterControl({
  ...
  const isGrid = field.fields?.some((sf) => sf.kind === "image");
  ...
  return (
    <div className="mt-6">
      ...
      <div className={isGrid ? "grid grid-cols-1 md:grid-cols-2 gap-6 mt-4" : "space-y-6"}>
        {items.map((item, index) => (
          <div key={item.id} className={isGrid ? "rounded-md border border-slate-200 bg-slate-50 p-4" : "mt-4 rounded-md border border-slate-200 bg-slate-50 p-4"}>
            ...
            <div className="space-y-4">
              ...
              {(() => {
                ...
                return (
                  <div className={`flex flex-col gap-4 ${isGrid ? "" : "sm:flex-row"}`}>
