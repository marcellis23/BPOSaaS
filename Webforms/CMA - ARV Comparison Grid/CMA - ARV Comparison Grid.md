...
<head>  
    ...
    <title>Sold Comparables (After-Repair) Analysis</title>  
...
<body class="bg-gray-50 text-gray-800 font-sans p-4 md:p-8">  
    ...
    <div class="max-w-7xl mx-auto">  
        ...
        <header class="mb-8 text-center md:text-left md:flex md:justify-between md:items-end">  
            <div>  
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Sold Comparables (After-Repair) Analysis</h1>  
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">  
            ...
            <!-- Summary Field -->  
            <div>  
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Summary of the Subject’s Projected/Proposed Use & Condition</label>  
                <textarea id="subject-activity-summary" rows="3" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-brand-500 focus:border-brand-500 sm:text-sm p-2 border" placeholder="Enter notes on projected use, proposed renovations, and condition..."></textarea>  
            </div>  
                ...
                <table class="cma-table">  
                    <thead>  
                        <tr class="bg-gray-800 text-white text-left text-sm uppercase tracking-wider">  
                            ...
                            <th class="p-4 w-1/5 bg-gray-800">Sold Comp 1</th>  
                            <th class="p-4 w-1/5 bg-gray-800">Sold Comp 2</th>  
                            <th class="p-4 w-1/5 bg-gray-800">Sold Comp 3</th>  
                        </tr>  
                    </thead>  
        ...
        <!-- F. Sold Comparables (After-Repair) Analysis Summary -->  
        <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-10">  
            <h2 class="text-lg font-bold text-gray-900 mb-4 flex items-center">  
                ...
                Sold Comparables (After-Repair) Analysis Summary  
            </h2>  
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">  
                <div>  
                    <label class="block text-sm font-bold text-gray-900 mb-2">Most Probable (After-Repair) Sale Price ($)</label>  
                    <input type="number" id="probable-sale-price" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-brand-500 focus:border-brand-500 sm:text-sm p-3 border text-lg font-semibold text-green-700" placeholder="e.g. 500000">  
                </div>  
                <div>  
...
<script>  
    ...
    const gridConfig = [  
        ...
        { label: 'Sale Date', idBase: 'sale-date', subType: 'text', compType: 'date', subVal: 'N/A' },  
        { label: 'Sale Price ($)', idBase: 'sale-price', subType: 'text', compType: 'number', subVal: 'N/A', extra: 'data-calc="price"' },  
        { label: 'Days on Market', idBase: 'dom', subType: 'text', compType: 'number', subVal: 'N/A' },  
        { label: 'List-to-Sales Ratio', idBase: 'list-to-sale', subType: 'display', subVal: '-', compType: 'calc', extraClass: 'calc-field' },  
        ...
        { label: 'Sale Concessions', idBase: 'adj-concessions', subType: 'display', subVal: '-', compType: 'number', extra: 'data-adj="true"' },  
        { label: 'Financing Concession', idBase: 'adj-financing', subType: 'display', subVal: '-', compType: 'number', extra: 'data-adj="true"' },  
        ...
        { label: 'Adjusted Sale Price', idBase: 'res-final-price', subType: 'display', subVal: '-', compType: 'calc', extraClass: 'final-price-field' },  
    ];  
    ...
    function calculate() {  
        ...
        soldCompIds.forEach(cid => {  
            const price = getNum(`${cid}-sale-price`);  
            const gla = getNum(`${cid}-gla`);  
            const dom = getNum(`${cid}-dom`);  
            const origPrice = getNum(`${cid}-orig-list-price`);  
            ...
            // 1a. List-to-Sales Ratio  
            if (price && origPrice) {  
                setVal(`${cid}-list-to-sale`, fmtPct((price / origPrice) * 100));  
            } else {  
                setVal(`${cid}-list-to-sale`, '-');  
            }  
    ...
    async function generatePDF() {  
        ...
        try {  
            // Title  
            doc.setFontSize(18);  
            doc.text("Sold Comparables (After-Repair) Analysis", 14, 15);  
            ...
            if(subjectSummary) {  
                currentY += 8;  
                const splitSummary = doc.splitTextToSize(`Projected/Proposed Use & Condition: ${subjectSummary}`, 270);  
            ...
            doc.text("Comp 1", startX + labelWidth + colWidth + 2, y + 5);  
            doc.text("Comp 2", startX + labelWidth + (colWidth * 2) + 2, y + 5);  
            doc.text("Comp 3", startX + labelWidth + (colWidth * 3) + 2, y + 5);  
            y += rowHeight;  
            ...
            // Sold Comparables Analysis Summary  
            const probablePrice = document.getElementById('probable-sale-price').value;  
            ...
            if(probablePrice || projectedDom || fullSummary) {  
                ...
                doc.setFont(undefined, 'bold');  
                doc.setFontSize(14);  
                doc.text("Sold Comparables (After-Repair) Analysis Summary", 14, y);  
                doc.setFontSize(10);  
                y += 8;  
  
                if (probablePrice || projectedDom) {  
                    doc.setFont(undefined, 'bold');  
                    let priceStr = probablePrice ? `Most Probable (After-Repair) Sale Price: $${Number(probablePrice).toLocaleString()}` : '';  
